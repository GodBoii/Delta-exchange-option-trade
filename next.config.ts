    ...
    return [
      {
        ...
        headers: [
          ...
          { key: "Content-Security-Policy", value: `default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' ${supabaseOrigin} ${apiOrigin} ${binanceApiOrigin} ${binanceWsOrigin} ${convexOrigin} ${convexWsOrigin} http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* ws://[::1]:*; frame-ancestors 'none'; base-uri 'self'; form-action 'self'` }
        ]
      }
    ];
  }
