...
export const publish = mutation({
  args: { secret: v.string(), runId: v.string(), candidates: v.array(v.object({ id: v.string(), version: v.number() })),
    ...
    supporting: v.array(v.string()), invalidation: v.array(v.string()), snapshotId: v.string() },
  handler: async (ctx, args) => {
    ...
    if (!args.candidates.length || args.candidates.length > 20 || new Set(args.candidates.map(item => item.id)).size !== args.candidates.length) throw new ConvexError("Invalid ranked candidates");
    ...
    await put(ctx, "strategy_proposals", { id, user_id: sharedUserId, agent_run_id: args.runId, strategy_id: null,
      ...
      name: "Shared ranked strategy decision", candidates, activation_time: args.activation, proposal_expiry: args.expiry,
...
export const allocate = mutation({
  args: { secret: v.string(), decisionId: v.string(), userId: v.string(), savedId: v.string(), savedVersion: v.number(), definitionJson: v.string(), exit: v.string() },
  handler: async (ctx, args) => {
    ...
    if (!saved || saved.deleted || !saved.enabled_for_ai || saved.version !== args.savedVersion || saved.user_id !== null) throw new ConvexError("Built-in strategy changed");
    ...
    const existing = await ctx.db.query("strategy_proposals").withIndex("by_relation", q => q.eq("relation", args.decisionId)).collect();
    const allocated = existing.find(item => item.owner === args.userId);
    ...
    await put(ctx, "strategies", { id: strategyId, user_id: args.userId, name: saved.name, saved_strategy_id: saved.id,
      status: "scheduled", definition_json: parseRow(args.definitionJson), entry_at: plan.activation_time, exit_at: args.exit,
...
export const manual = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const recent = await ctx.db.query("automation_agent_runs").withIndex("by_owner_created", q => q.eq("owner", sharedUserId).gte("created", Date.now() - 300000)).order("desc").first();
