-- Add the daily pre-expiry review to trigger validation and follow-up boundaries.
begin;

alter table public.automation_agent_runs
  drop constraint automation_agent_runs_trigger_check;
alter table public.automation_agent_runs
  add constraint automation_agent_runs_trigger_check
  check (trigger in (
    'manual', 'asia_session', 'london_session', 'new_york_session',
    'pre_expiry', 'agent_follow_up', 'activation_recheck'
  ));
  ...
  with ranked_follow_ups as (
    ...
    join lateral (
      ...
      where fixed.user_id = follow_up.user_id
        ...
        and fixed.trigger in ('asia_session', 'london_session', 'new_york_session', 'pre_expiry')
